...
Durations are best-effort and flexible to accommodate new findings; no phase begins before the prior phase's exit criteria are satisfied (Rule 9, phase-gated build order).
...
- Minimal HITL review endpoint for human sign-off.
  **Deliverables:** governance module; audit log store; review endpoint; a showcase-interface slice (ADR-009) adding the audit record and HITL view, including the preset scenario engineered to trigger the deterministic fallback.
  **Exit criteria:** 100% audit-record completeness across a test batch spanning both escalated and auto-approved scenarios (PRD §11); fallback triggers correctly under a simulated agent failure; the fallback preset scenario visibly demonstrates this in the showcase interface.

...
**Status: RESOLVED — Approved by Sebastián Garrido Arévalo on August 15, 2026.**
This is a living document. Decisions marked _No User Input Required_ were recorded for completeness and ADR discipline. All other decisions have been reviewed and approved. Outcomes have been recorded as new ADRs (ADR-004 through ADR-008) in `reports/docs/architecture/system_design.md`. The entries below are marked `RESOLVED` in place rather than deleted, preserving the trade-off reasoning and deliberation trail for future reference.
...
| ----- | ------------------------------------------ | ------------------------------ | ------------ | ----------------------------------------------------------------------------------------------------- |
| P1-D1 | Python package layout & environment        | Modularity                     | **RESOLVED** | D1a: Opt B (namespaced), D1b: Opt A (hatchling), D1c: Opt B (floor)                                   |
| P1-D2 | `params.yaml` schema & validation strategy | Modularity, cost               | **RESOLVED** | D2a: Opt B (nested), D2b: Opt A (Pydantic), D2c: No secrets                                           |
| P1-D3 | Great Expectations suite architecture      | Modularity, cost               | **RESOLVED** | D3a: Opt A (GX Core files), D3b: Opt A (JSON), D3c: Opt A (fixtures)                                  |
| P1-D4 | DVC pipeline skeleton                      | Cost, modularity               | **RESOLVED** | D4a: Local remote, D4b: Opt B (fine-grained stages)                                                   |
| P1-D5 | CI skeleton design                         | Cost, modularity, dev-velocity | **RESOLVED** | D5a: Opt A (single workflow), D5b: No secrets, D5c: Opt A (INV-8 check), D5d: Opt A (pytest skeleton) |
...
#### P1-D1a — Flat vs. namespaced `src` layout `[RESOLVED — Option B]`
...
- **Option B — Namespaced (APPROVED).** Introduce `src/aegis/` as the actual package, with everything else nested under it (`src/aegis/gateway/`, `src/aegis/agents/`, ...). Imports become `from aegis.gateway import ...`.
...
**Outcome:** **RESOLVED — Option B.** `src/aegis/` adopted as the package root (recorded in ADR-004).

---

#### P1-D1b — Build backend `[RESOLVED — Option A]`

- **Option A — Hatchling (APPROVED).** Widely used, well-documented, the most common pairing with `uv` in current tutorials and templates.
...
**Outcome:** **RESOLVED — Option A.** `hatchling` selected as the PEP 517 build backend in `pyproject.toml` (recorded in ADR-004).

---

#### P1-D1c — Python version pin `[RESOLVED — Option B]`

- **Option A — Exact patch pin** (e.g., `==3.12.x` in `.python-version`, matched in `pyproject.toml`).
- **Option B — Minor-version floor (APPROVED).** (`>=3.12,<3.13`).
...
**Outcome:** **RESOLVED — Option B.** Minor-version floor `>=3.12,<3.13` specified with `3.12` pinned in `.python-version` (recorded in ADR-004).
...
#### P1-D2a — Section structure `[RESOLVED — Option B]`
...
- **Option B — Nested by domain (APPROVED).** Top-level sections mirroring the architecture tiers — `gateway:`, `tier1_ml:`, `tier2_agents:`, `governance:`, `data_contracts:`, `dvc:` — with related settings grouped under each.
...
**Outcome:** **RESOLVED — Option B.** Nested section structure by architectural domain adopted (recorded in ADR-005).

---

#### P1-D2b — Validation mechanism `[RESOLVED — Option A]`

- **Option A — Pydantic settings model (APPROVED).** Define a `BaseModel` (or `BaseSettings`) schema that `params.yaml` is loaded into and validated against at startup; malformed or missing values fail fast with a typed error.
...
**Outcome:** **RESOLVED — Option A.** Pydantic-based configuration model with strict type validation at load time (recorded in ADR-005).

---

#### P1-D2c — Secrets exclusion _[RESOLVED — Invariant / No user input required]_
...
**Outcome:** **RESOLVED.** Zero-secret invariant strictly enforced for `params.yaml` (recorded in ADR-005).
...
#### P1-D3a — GX API/version to target `[RESOLVED — Option A]`
...
- **Option A — Current stable GX Core (open-source), file-based expectation suites (APPROVED).** No GX Cloud account, no external service dependency — suites stored as version-controlled files in the repo.
...
**Trade-off:** Option A, unambiguously — GX Cloud has no upside for a solo, cost-conscious, local-first project and a real downside (external dependency, likely credentials to manage). The only genuinely open question is the exact current API syntax within GX Core, which I'll verify against current docs before implementation rather than asserting here.
...
**Outcome:** **RESOLVED — Option A.** Open-source GX Core file-based expectation suites targeting current stable API (recorded in ADR-006).

---

#### P1-D3b — Suite storage format `[RESOLVED — Option A]`

- **Option A — GX's native JSON expectation-suite files (APPROVED)**, committed to `data_contracts/`.
...
**Outcome:** **RESOLVED — Option A.** Native JSON expectation-suite files committed to `data_contracts/` for both data pipelines (recorded in ADR-006).

#### P1-D3c — Fixture/test-data strategy `[RESOLVED — Option A]`

- **Option A — Hand-crafted minimal fixtures (APPROVED).** A handful of small, deliberately constructed CSV/JSON rows per suite — one clearly valid, one or two clearly malformed in a specific, named way (missing metadata field, negative exposure value, duplicate chunk, a leaked post-treatment column) — committed to `data_contracts/fixtures/`.
...
**Outcome:** **RESOLVED — Option A.** Hand-crafted minimal valid and malformed fixtures in `data_contracts/fixtures/` (recorded in ADR-006).
...
#### P1-D4a — Remote storage backend _[RESOLVED — Invariant / No user input required]_
...
**Outcome:** **RESOLVED.** Local filesystem cache/remote configured (recorded in ADR-007).

---

#### P1-D4b — Pipeline stage granularity `[RESOLVED — Option B]`
...
- **Option B — Fine-grained (APPROVED).** Separate stages for raw ingest, GX validation, and validated/versioned output, so the DVC DAG shows validation as its own explicit node.
...
**Outcome:** **RESOLVED — Option B.** Fine-grained DVC pipeline stages with explicit GX validation nodes in the DAG (recorded in ADR-007).
...
#### P1-D5a — Workflow structure `[RESOLVED — Option A]`

- **Option A — Single combined workflow (APPROVED)** (`.github/workflows/ci.yml`) running lint → type-check → module-size check → GX gates (against fixtures) → tests as sequential jobs/steps, one pass/fail status.
...
**Outcome:** **RESOLVED — Option A.** Single unified GitHub Actions workflow in `.github/workflows/ci.yml` (recorded in ADR-008).

---

#### P1-D5b — Secrets requirement this phase _[RESOLVED — Invariant / No user input required]_
...
**Outcome:** **RESOLVED.** Zero secrets required for Phase 1 CI (recorded in ADR-008).

---

#### P1-D5c — Wire the INV-8 module-size checker into CI now? `[RESOLVED — Option A]`

- **Option A — Yes, now (APPROVED).** Add `scripts/check_module_size.py` and its CI step in Phase 1, even though `src/` is nearly empty.
...
**Outcome:** **RESOLVED — Option A.** `scripts/check_module_size.py` implemented and enforced as a blocking CI gate from Day 1 (recorded in ADR-008).

---

#### P1-D5d — Minimal `pytest` skeleton now, or defer? `[RESOLVED — Option A]`

- **Option A — Stand up a minimal skeleton now (APPROVED)**, with a single trivial test (e.g., confirming the `params.yaml` schema loads and validates) so the CI "tests" stage isn't vacuous.
...
**Outcome:** **RESOLVED — Option A.** Minimal test suite initialized in `tests/unit/` testing configuration validation (recorded in ADR-008).

---

## Part D — What Happens After Approval

All Phase 1 architectural decisions are now finalized and approved:
